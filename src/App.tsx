...
function ApplyPage({ entries }: { entries: any[] }) {
  ...
  const pastData = entries.slice(-20).map((e: any) => ({
    title: e.title,
    tags: e.tags || [],
    q1: e.answers?.q1 || "",
    q4: e.answers?.q4 || "",
  ...
  return (
    <main style={{ maxWidth: 720, margin: "0 auto", padding: "24px 20px 48px" }}>
      <div style={{ fontSize: 34, fontWeight: 700, letterSpacing: "-0.5px", marginBottom: 4 }}>Apply</div>
      <div style={{ fontSize: 15, color: "#8E8E93", marginBottom: 24 }}>図鑑から「あなたらしいテーマ」を提案します</div>

      <div style={{ background: "#fff", borderRadius: 14, padding: "14px 16px", marginBottom: 12 }}>
        <div style={{ fontSize: 12, color: "#8E8E93", fontWeight: 600, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>お題・課題</div>
        <textarea value={subject} onChange={e => { setSubject(e.target.value); setThemes([]); setRoadmap(null); setSelectedTheme(null); }} placeholder="課題のお題を入力（例：環境問題、就活、地域活性化）" rows={2} style={{ width: "100%", border: "none", fontSize: 15, background: "transparent", fontFamily: "inherit", resize: "none", outline: "none", lineHeight: 1.6 }} />
      </div>

      <button onClick={generateThemes} disabled={loading || !subject} style={{ width: "100%", background: loading && !selectedTheme ? "#5856D6" : loading ? "#E5E5EA" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", color: loading && !selectedTheme ? "#fff" : loading ? "#8E8E93" : "#fff", border: "none", padding: "16px", borderRadius: 14, fontSize: 15, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit", marginBottom: 20 }}>
        {loading && !selectedTheme ? "✨ あなたらしいテーマを考えています..." : "✨ あなたらしいテーマを提案してもらう"}
      </button>
      ...
      {themes.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
          <div style={{ fontSize: 13, color: "#8E8E93", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 4 }}>あなたらしいテーマ案（タップして選ぶ）</div>
          {themes.map((t: any, i: number) => (
            <div key={i} onClick={() => generateRoadmap(t)} style={{ background: selectedTheme?.title === t.title ? "#007AFF" : "#fff", borderRadius: 14, padding: "16px 18px", cursor: "pointer", transition: "all 0.2s" }}>
              <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 4, color: selectedTheme?.title === t.title ? "#fff" : "#000" }}>{t.title}</div>
              <div style={{ fontSize: 14, color: selectedTheme?.title === t.title ? "rgba(255,255,255,0.85)" : "#3C3C43", marginBottom: 6 }}>{t.concept}</div>
              <div style={{ fontSize: 12, color: selectedTheme?.title === t.title ? "rgba(255,255,255,0.7)" : "#8E8E93" }}>なぜ合うか：{t.why}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {loading && selectedTheme && (
        <div style={{ textAlign: "center", padding: "20px", color: "#8E8E93" }}>✨ 制作プロセスを組み立てています...</div>
      )}

      {roadmap && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ fontSize: 13, color: "#8E8E93", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>制作プロセス</div>
          <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden" }}>
            {roadmap.steps?.map((s: any, i: number) => (
              <div key={i} style={{ padding: "14px 18px", borderBottom: i < roadmap.steps.length - 1 ? "0.5px solid #E5E5EA" : "none", display: "flex", gap: 14, alignItems: "flex-start" }}>
                <div style={{ width: 28, height: 28, borderRadius: 99, background: "#007AFF", color: "#fff", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{s.step}</div>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 2 }}>{s.title}</div>
                  <div style={{ fontSize: 13, color: "#8E8E93" }}>{s.action}</div>
                </div>
              </div>
            ))}
          </div>

          <div style={{ fontSize: 13, color: "#8E8E93", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>図鑑から使えるアイデア</div>
          {roadmap.ideas?.map((idea: any, i: number) => (
            <div key={i} style={{ background: "#fff", borderRadius: 14, padding: "14px 18px" }}>
              <div style={{ fontSize: 11, color: "#007AFF", fontWeight: 600, marginBottom: 4 }}>📌 {idea.source} より「{idea.element}」</div>
              <div style={{ fontSize: 15, fontWeight: 600 }}>→ {idea.application}</div>
            </div>
          ))}

          {roadmap.concept && (
            <div style={{ background: "linear-gradient(135deg, #667eea15 0%, #764ba215 100%)", borderRadius: 14, padding: "20px 18px", border: "1.5px solid #667eea33" }}>
              <div style={{ fontSize: 12, color: "#667eea", fontWeight: 600, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.05em" }}>✨ 最終コンセプト案</div>
              <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 10 }}>{roadmap.concept.title}</div>
              <div style={{ fontSize: 14, lineHeight: 1.7, color: "#3C3C43" }}>{roadmap.concept.description}</div>
            </div>
          )}
        </div>
      )}
      ...
      {entries.length === 0 && (
        <div style={{ textAlign: "center", padding: "40px 20px", color: "#8E8E93" }}>
          ...
          <div style={{ fontSize: 15, color: "#8E8E93" }}>まず作品を解剖して図鑑を作ろう</div>
        </div>
      )}
    </main>
  );
